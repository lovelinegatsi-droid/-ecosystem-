...
bool Window::Initialize() { 
    if (SDL_Init(SDL_INIT_VIDEO) != 0) { 
        std::cerr << "
 ❌
 Erreur SDL_Init: " << SDL_GetError() << std::endl; 
        return false; 
    }
    ...
    if (!mWindow) { 
        std::cerr << "
 ❌
 Erreur création fenêtre: " << SDL_GetError() << std::endl;
        SDL_Quit(); 
        return false; 
    }
    ...
    if (!mRenderer) { 
        std::cerr << "
 ❌
 Erreur création renderer: " << SDL_GetError() << std::endl
    ...
    std::cout << "
 ✅
 Fenêtre initialisée: " << mTitle << " (" << mWidth << "x" << m
    return true; 
} 
// 
�
